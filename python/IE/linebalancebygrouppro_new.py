...
sql_ds_hc=('SELECT hc.idx AS employee,hc.NAME,hc.line,hc.shift,hc.namegroup,hc.work_hs,ds.OPERATION,ds.EFF,ds.Style'
            ...
            +'emp.WEEK,emp.line_new AS line,emp.ML,emp.shift_new AS shift,gr.groupname AS namegroup,if((INSTR(emp.shift,"MAT")>0 OR INSTR(emp.shift,"CHILD")>0 OR INSTR(emp.shift,"OT")>0),384,444) '
...
if len(ds)==0 or week_cbc==0:
    # print("normal")
    sql_ds=('SELECT RIGHT(e.id,5) as employee,e.NAME,e.line_new as line,e.shift_new as shift,g.groupname,if((INSTR(e.shift_new,"MAT")>0 OR INSTR(e.shift_new,"CHILD")>0 OR INSTR(e.shift,"OT")>0),384,444)  as work_hs FROM linebalancing.setup_emplist e INNER JOIN linebalancing.web_ie_location g ON e.Line_new=g.line WHERE e.ML="" AND e.line_new<"line 354" '
    ...
    while i< len(ds):
        ...
        eff_emp=36
        week_ie=week
        web_ie=pd.read_sql('select operation_name,eff,ie,week from web_ie_update where employee="'+emp+'" and style_detail="'+style_detail+'" and range_size="'+size+'" and (week="'+str(week)+'" or week="'+str(week-1)+'") and date_update>"20220101" order by week desc;',engine_hbi_linebalancing)
        if len(web_ie)>0:
            opr=web_ie.iloc[0,0]
        ...
        if eff_emp==36 or week_ie<week:
            ...
            if opr=="ie_assign":
                sql_op=(
                        'SELECT sample.operation_name,COUNT(sample.DATE) rf,max(sample.DATE) AS ld FROM '
                        ...
                        +'employee="'+emp+'" AND style_detail="'+style_detail+'" ORDER BY DATE DESC LIMIT 21) AS sample GROUP BY sample.operation_name ORDER BY ld desc;'
                    )
                # print(sql_op)
            ...
            if opr=="ie_assign" or du>15:
                refer=1
                sql_op_op=(
                            'SELECT operation_name,COUNT(DATE) AS rf,max(date) as ld from '
                            ...
                            +'employee="'+emp+'" AND Workcenter="'+wc+'" ORDER BY DATE DESC LIMIT 21) AS temp GROUP BY operation_name ORDER BY ld desc;'
                )
            ...
            if opr!="ie_assign":
                style_referx=(
                        ...
                        +'WHERE operation_name is not null and float_eff IS NOT NULL AND FLOAT_eff<300 and employee="'+emp+'" AND Workcenter="'+wc+'" ORDER BY DATE DESC LIMIT 21)AS sample '
            ...
            if (opr!="ie_assign" and refer==0):
                try:
                    sql_eff1=(
                                'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND '+dkstyle+' AND DATE<"'+str(dlimit)+'" ORDER BY DATE DESC LIMIT 21;'
                    )
                    dt_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
                except:
                    sql_eff1=(
                                'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size_ex+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND '+dkstyle+' AND DATE<"'+str(dlimit)+'" ORDER BY DATE DESC LIMIT 21;'
                    )
                    dt_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
            ...
            if (opr!="ie_assign" and refer==0):
                try:
                    sql_eff1=(
                                'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND '+dkstyle+' AND DATE<"'+str(dlimit)+'" AND DATE<"20211104" ORDER BY DATE DESC LIMIT 21;'
                    )
                    q4_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
                except:
                    sql_eff1=(
                                'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size_ex+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND '+dkstyle+' AND DATE<"'+str(dlimit)+'"  AND DATE<"20211104" ORDER BY DATE DESC LIMIT 21;'
                    )
                    q4_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
            ...
            if (opr!="ie_assign"):
                ...
                wc_eff1=[]
                sql_eff1=(
                            'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND workcenter="'+wc+'" AND DATE<"'+str(dlimit)+'" ORDER BY DATE DESC LIMIT 21;'
                )
                wc_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
                if len(wc_eff1)==0:
                    sql_eff1=(
                                'SELECT operation_name,size,style_detail,workcenter,DATE,float_eff FROM bundle_group_by_employee_detail WHERE '+range_size_ex+' and float_eff IS NOT NULL AND FLOAT_eff<300 and operation_name="'+opr+'" and employee="'+emp+'" AND workcenter="'+wc+'" AND DATE<"'+str(dlimit)+'" ORDER BY DATE DESC LIMIT 21;'
                    )
                    wc_eff1=pd.read_sql(sql_eff1,engine_hbi_linebalancing)
    ...
    while i<len(cbc1):
        if cbc1.iloc[i,0]!="NDH" and cbc1.iloc[i,0]!="CLIP THREAD" and cbc1.iloc[i,0]!="ie_assign" and cbc1.iloc[i,0]!="CASING" and cbc1.iloc[i,0]!="PAD PRINT" and cbc1.iloc[i,0]!="MAKE BAND" and cbc1.iloc[i,0]!="PACKING" and cbc1.iloc[i,0]!="MOVER 3" and cbc1.iloc[i,0]!="SEAM WAISTBAND":
            if mi==cbc1.iloc[i,6]:
                ...
                cbc1.iloc[i,6]=3*(float(cbc1.iloc[i,7])*444*cbc1.iloc[i,10]-float(cbc1.iloc[i,8])*444*cbc1.iloc[i,2]+float(cbc1.iloc[i,3])*cbc1.iloc[i,2])/cbc1.iloc[i,5]/100
            else:
                if ma==cbc1.iloc[i,6]:
...
kq={'Min':str(round(mi,0)),'Max':str(round(ma,0)),'Rate':str(round(target,2)),'Eff':str(eff_cbc),'Head_Count':str(len(ds_final)-NDH),'NDH':str(NDH),'week':week,'T18':str(t18),'T18PL':str(t18pl),'T24':str(t24),'DLM':dlimit,'WC':wc}

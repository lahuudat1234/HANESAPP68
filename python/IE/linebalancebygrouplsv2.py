...
sql_ds_hc=('SELECT hc.idx AS employee,hc.NAME,hc.line,hc.shift,hc.namegroup,hc.work_hs,ds.OPERATION,ds.EFF,ds.Style'
            ...
            +'namegroup,work_hs,OPERATION,EFF,style_refer as Style,Var,ieu as IE from employee_eff_data_ie_setup_longshift '
            ...
            +'work_hrs as work_hs FROM linebalancing.setup_emplist_ls emp inner JOIN (SELECT line,groupname FROM web_ie_location lc WHERE '
            +'groupname="'+group+'") AS gr ON (emp.line_new=gr.line) WHERE (emp.shift_new LIKE"'+shift+'%") AND emp.week="'+str(week)+'" '
            +' GROUP BY idx) AS hc ON ds.employee=hc.idx WHERE hc.idx IS NOT NULL GROUP BY hc.idx;')
...
if len(ds)==0 or week_cbc==0:
    # print("normal")
    sql_ds=('SELECT RIGHT(e.id,5) as employee,e.NAME,e.line_new as line,e.shift_new as shift,g.groupname,work_hrs as work_hs FROM linebalancing.setup_emplist_ls e INNER JOIN linebalancing.web_ie_location g ON e.Line_new=g.line WHERE e.ML="" AND e.line_new<"line 354" '
    ...
    while i< len(ds):
        ...
        web_ie=pd.read_sql('select operation_name,eff,ie from web_ie_update where employee="'+emp+'" and style_detail="'+style_detail+'" and range_size="'+size+'" and week="'+str(week)+'";',engine_hbi_linebalancing)
        if len(web_ie)>0:
            opr=web_ie.iloc[0,0]
...
while i<len(cbc1):
    ...
    sql_eff2='select eff from web_ie_operation2 where week="'+str(week)+'" and groupcbc="'+group+'" and style="'+style_detail+'" and size="'+size+'" and shift="'+shift+'" and operation="'+OP+'";'
    # print(sql_sam)
    dt_eff2=pd.read_sql(sql_eff2,engine_hbi_linebalancing)
    engine_hbi_linebalancing.dispose()
    if len(dt_eff2)>0:
...
while target<=0.95 and lp<200:
    ...
    while i<len(cbc1):
        if cbc1.iloc[i,0]!="NDH" and cbc1.iloc[i,0]!="ie_assign" and cbc1.iloc[i,0]!="CASING" and cbc1.iloc[i,0]!="PAD PRINT" and cbc1.iloc[i,0]!="MAKE BAND" and cbc1.iloc[i,0]!="PACKING" and cbc1.iloc[i,0]!="MOVER 3" and cbc1.iloc[i,0]!="SEAM WAISTBAND":
            if mi==cbc1.iloc[i,6]:
                cbc1.iloc[i,7]=cbc1.iloc[i,7]+0.05  
                ...
                if ma==cbc1.iloc[i,6]:
                    cbc1.iloc[i,8]=cbc1.iloc[i,8]+0.05
                    cbc1.iloc[i,6]=3*(float(cbc1.iloc[i,7])*442*cbc1.iloc[i,10]-float(cbc1.iloc[i,8])*442*cbc1.iloc[i,2]+float(cbc1.iloc[i,3])*cbc1.iloc[i,2])/cbc1.iloc[i,5]/100            
        i=i+1
    # check balance rate
    i=0
...
while i<len(dt_opst):
    dc=0
    if(dt_opst.iloc[i,0]!="MAKE BAND" and dt_opst.iloc[i,0]!="PAD PRINT"):
        j=0
        while j<len(cbc1):
...
if ie_cbc!="" and week_cbc>0:
    ...
    while i<len(ds_final):
        ...
        sql_ds=('replace into employee_eff_data_ie_setup_longshift (keye,week,groupline,employee,name,shift,work_hs,style_detail,operation,size,eff,style_refer,var,ieu,iecbc) values('

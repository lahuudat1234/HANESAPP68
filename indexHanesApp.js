...
app.get("/chinh-gio", function (req, res) {
    console.log("GET /chinh-gio");
    res.render("TemChung/chinh-gio");
    res.end
})

app.get("/tem-chung/load-data-by-date", function (req, res) {
    con5.getConnection(function (err, connection) {
        if (err) {
            throw err;
        }
        sql_c = `SELECT *, DATE_FORMAT(time_in,'%Y-%m-%d %T') AS time_in_str, DATE_FORMAT(time_out,'%Y-%m-%d %T') AS time_out_str, hours  
        AS diff FROM cutting_system.time_scan t where t.date = '${req.query.date}' order by time_in desc`
        connection.query(sql_c, function (err, result, fields) {
            if (err) throw err;
            connection.release();
            res.send(result);
...
app.post("/update-time", function (req, res) {
    con5.getConnection(function (err, connection) {
        ...
        sql = (`update cutting_system.time_scan set time_in = '${req.body.timein}', time_out = '${req.body.timeout}', hours = ROUND(TIMESTAMPDIFF(SECOND, '${req.body.timein}', '${req.body.timeout}') / 3600, 2), 
        time_in_old = '${req.body.timeinold}', time_out_old = '${req.body.timeoutold}', user_update = '${req.body.user_update}'  where keyx = '${req.body.keyx}' `)
        connection.query(sql, function (err, result, fields) {
            if (err) throw err;
        });
        connection.release();
